...
namespace Lab1
{
    ...
    class V3DataArray : V3Data
    {
        ...
        public override double MaxDistance
        { 
            get { return Math.Sqrt((n_x - 1) * step_x * (n_x - 1) * step_x + (n_y - 1) * step_y * (n_y - 1) * step_y);  }
        }
        public override string ToString()
        {
        ...
        {
            string s = $"Typename: {this.GetType()}; str: {str}; date: [{date}]; count = {Count}; maxdistance = {MaxDistance.ToString(format)};";
            s += $" n_x = {n_x.ToString(format)}; n_y = {n_y.ToString(format)}; step_x = {step_x.ToString(format)}; step_y = {step_y.ToString(format)}";
            s += "\n" + "Array:";
            for (int i = 0; i < n_x; i++)
            {
                for (int j = 0; j < n_y; j++)
